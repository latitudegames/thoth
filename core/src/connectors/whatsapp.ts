...
export async function onMessage(msg, messageResponseHandler) {
    ...
    if (minsDiff > 12 || (minsDiff <= 12 && hoursDiff > 1)) return
    ...
    if (msg.chat.type == 'supergroup') {
        ...
        if (!msg.from.is_bot) {
            ...
            prevMessageTimers[msg.chat.id] = setTimeout(
        ...
        const isMention =
            ...
            content.includes('@' + customConfig.instance.get('whatsappBotName'))
        const otherMention =
            msg.entities !== undefined &&
...
export function sentMessage(user) {
    ...
    conversation[user] = setTimeout(function () {
