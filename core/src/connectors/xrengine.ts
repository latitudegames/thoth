...
export function onMessageDeleted(chatId, messageId) {
    if (
        messageResponses[chatId] !== undefined &&
        messageResponses[chatId][messageId] !== undefined
    ) {
...
export async function saveIfHandled(
    chatId,
    messageId,
    sender,
    content,
...
export function isInRange(user) {
    return (
        UsersInRange[user] !== undefined ||
        UsersInHarassmentRange[user] !== undefined ||
        UsersInIntimateRange[user] !== undefined
    )
}

export async function handleMessages(messages, bot) {
    for (let i = 0; i < messages.length; i++) {
        if (messages[i].text.includes('[') && messages[i].text.includes(']'))
            continue
        else if (
            messages[i].text.includes('joined the layer') ||
            messages[i].text.includes('left the layer') ||
            messages[i].text.length === 0
        )
            continue
            ...
            messages[i].senderName === bot.name ||
            (messages[i].sender !== undefined &&
                messages[i].sender.id === bot.userId) ||
        ...
        await wasHandled(
            ...
            async () => {
                ...
                const minsDiff = Math.ceil((diff - hoursDiff) / 60)
                if (minsDiff > 12 || (minsDiff <= 5 && hoursDiff > 1)) {
                    ...
                    saveIfHandled(
                        messages[i].channelId,
                        messages[i].id,
                        messages[i].senderName !== undefined
                ...
                if (prevMessageTimers[messages[i].channelId] !== undefined)
                    clearTimeout(prevMessageTimers[messages[i].channelId])
                prevMessageTimers[messages[i].channelId] = setTimeout(
                    () => (prevMessage[messages[i].channelId] = ''),
                    120000
                )
...
async function createXREngineClient() {
    const XRENGINE_URL = await customConfig.instance.get('xrEngineURL')
    if (!XRENGINE_URL) {
        return console.log('Skipping XREngine startup no XRENGINE_URL set')
    }
...
class XREngineBot {
    ...
    async instanceMessages() {
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            const sender = messages[i].sender.name
    ...
    async launchBrowser() {
        ...
        this.page.on('console', message => {
            if (message.text().startsWith('scene_metadata')) {
                ...
                } else console.log(`invalid metadata length ${data.length}: ${data}`)
            ...
            if (this.autoLog) console.log('>> ', message.text())
        })
