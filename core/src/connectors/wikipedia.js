...
} from "../superreality/makeCompletionRequest.js";
...
export async function createWikipediaAgent(speaker, name, personality, facts) {
  try {
    ...
    let stop = Date.now()
    console.log(`Time Taken to execute loaded data from wikipedia = ${(stop - start) / 1000} seconds`);
    start = Date.now()
    ...
    stop = Date.now()
    console.log(`Time Taken to execute openai request 2 = ${(stop - start) / 1000} seconds`);
    start = Date.now()
