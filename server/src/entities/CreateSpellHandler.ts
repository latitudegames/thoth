...
export const CreateSpellHandler = async (props: { spell: any; version: string; }) => {
    ...
    async function spellHandler(
        ...
        // TO FIX
        // 1. Look up rete.js task plugin -- https://github.com/retejs/task-plugin
        // 2. Look up functionality of needReset -- if all tasks are reset, this will work
        // BUT it doesn't because some nodes need to note be reset, or they all need to be reset at same time
        // We could add all resettable nodes to global list, and the reset them from their after all tasks run
        // 3. Look at _task.closed, this is important to tasks being "done"
        // 4. Look at task.worker, since this is where the actually happens
        // 5. Look at node.builder, which creates instances of nodes
        // 6. Look at anything named task or _task or output
